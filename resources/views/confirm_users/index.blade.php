...
@section('title', __('confirm_users.title'))
...
@section('content')
    <main class="dashboard_content dashboard_content-norightsidebar">
        <div class="mx_title">
            <h2 class="ttl">@lang('confirm_users.table_name')</h2>
        </div>
        <div class="dash_list dash_list-confirmation">
            <div class="row row-title">
                <div class="cell">
                    @lang('confirm_users.table_id')
                </div>
                <div class="cell">
                    @lang('confirm_users.table_id_telegram')
                </div>
                <div class="cell">
                    @lang('confirm_users.table_login')
                </div>
                <div class="cell">
                    @lang('confirm_users.table_action')
                </div>
            </div>
            @if($user_data)
                    ...
                    <div class="row">
                        ...
                        <div class="cell">
                            <div class="action_buttons">
                                <form action="{{ route('confirm_users.update', $item->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                ...
                                <form action="{{ route('confirm_users.destroy', $item->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
